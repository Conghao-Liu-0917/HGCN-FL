...
class HypbolicPool(torch.nn.Module):
    ...
    def forward(self, x, edge_index, edge_attr, batch=None):
        ...
        # x_tan = self.manifold.proj_tan0(self.manifold.logmap0(x, c=self.c), c=self.c)  # 将x映射回欧式空间
        x_information_score = self.calc_information_score(x, edge_index, edge_attr)  # 对每个节点评分
        score = torch.sum(torch.abs(x_information_score), dim=1)  # 评分绝对值求和
        ...
        if self.sample:
            ...
            # weights = (torch.cat([x[row], x[col]], dim=1) * self.att).sum(dim=-1)
            # weights = F.leaky_relu(weights, self.negative_slop) + new_edge_attr * self.lamb
            weights = torch.cat([self.manifold.logmap0(x[row], c=self.c), self.manifold.logmap0(x[col], c=self.c)], dim=1)
            weights = self.manifold.proj(self.manifold.expmap0(self.manifold.proj_tan0(weights, self.c), self.c), self.c)
            ...
            hyp_b = self.manifold.proj(self.manifold.expmap0(self.manifold.proj_tan0(new_edge_attr * self.lamb, self.c), self.c),self.c)
            # mobius_add
